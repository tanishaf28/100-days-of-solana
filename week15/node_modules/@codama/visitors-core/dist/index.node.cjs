...
function identityVisitor(options = {}) {
  ...
  if (keys.includes("arrayTypeNode")) {
    visitor.visitArrayType = function visitArrayType(node) {
      ...
      nodes.assertIsNode(count, nodes.COUNT_NODES);
      const item = visit2(this)(node.item);
      if (item === null) return null;
      nodes.assertIsNode(item, nodes.TYPE_NODES);
      return nodes.arrayTypeNode(item, count);
    };
  }
  ...
  if (keys.includes("enumEmptyVariantTypeNode")) {
    visitor.visitEnumEmptyVariantType = function visitEnumEmptyVariantType(node) {
      ...
      if (display) nodes.assertIsNode(display, "enumVariantDisplayNode");
      return nodes.enumEmptyVariantTypeNode(node.name, node.discriminator, { ...node, display });
    };
  }
  if (keys.includes("enumStructVariantTypeNode")) {
    visitor.visitEnumStructVariantType = function visitEnumStructVariantType(node) {
      ...
      nodes.assertIsNestedTypeNode(struct, "structTypeNode");
      const display = node.display ? visit2(this)(node.display) ?? void 0 : void 0;
      if (display) nodes.assertIsNode(display, "enumVariantDisplayNode");
      return nodes.enumStructVariantTypeNode(node.name, struct, node.discriminator, { ...node, display });
    };
  }
  if (keys.includes("enumTupleVariantTypeNode")) {
    visitor.visitEnumTupleVariantType = function visitEnumTupleVariantType(node) {
      ...
      nodes.assertIsNestedTypeNode(tuple, "tupleTypeNode");
      const display = node.display ? visit2(this)(node.display) ?? void 0 : void 0;
      if (display) nodes.assertIsNode(display, "enumVariantDisplayNode");
      return nodes.enumTupleVariantTypeNode(node.name, tuple, node.discriminator, { ...node, display });
    };
  }
  ...
  if (keys.includes("hiddenPrefixTypeNode")) {
    visitor.visitHiddenPrefixType = function visitHiddenPrefixType(node) {
      ...
      if (type === null) return null;
      nodes.assertIsNode(type, nodes.TYPE_NODES);
      return nodes.hiddenPrefixTypeNode(
        type,
        (node.prefix ?? []).map(visit2(this)).filter(nodes.removeNullAndAssertIsNodeFilter("constantValueNode"))
      );
    };
  }
  if (keys.includes("hiddenSuffixTypeNode")) {
    visitor.visitHiddenSuffixType = function visitHiddenSuffixType(node) {
      ...
      if (type === null) return null;
      nodes.assertIsNode(type, nodes.TYPE_NODES);
      return nodes.hiddenSuffixTypeNode(
        type,
        (node.suffix ?? []).map(visit2(this)).filter(nodes.removeNullAndAssertIsNodeFilter("constantValueNode"))
      );
    };
  }
  if (keys.includes("mapTypeNode")) {
    visitor.visitMapType = function visitMapType(node) {
      ...
      return nodes.mapTypeNode(key, value, count);
    };
  }
  if (keys.includes("numberTypeNode")) {
    visitor.visitNumberType = function visitNumberType(node) {
      const display = node.display ? visit2(this)(node.display) ?? void 0 : void 0;
      if (display)
        nodes.assertIsNode(display, [
          "amountNumberDisplayNode",
          "dateTimeNumberDisplayNode",
          "durationNumberDisplayNode"
        ]);
      return nodes.numberTypeNode(node.format, node.endian, { ...node, display });
    };
  }
  ...
  if (keys.includes("setTypeNode")) {
    visitor.visitSetType = function visitSetType(node) {
      ...
      if (item === null) return null;
      nodes.assertIsNode(item, nodes.TYPE_NODES);
      return nodes.setTypeNode(item, count);
    };
  }
  ...
  if (keys.includes("stringTypeNode")) {
    visitor.visitStringType = function visitStringType(node) {
      const display = node.display ? visit2(this)(node.display) ?? void 0 : void 0;
      if (display) nodes.assertIsNode(display, "stringDisplayNode");
      return nodes.stringTypeNode(node.encoding, { ...node, display });
    };
  }
  if (keys.includes("structFieldTypeNode")) {
    visitor.visitStructFieldType = function visitStructFieldType(node) {
      ...
      const display = node.display ? visit2(this)(node.display) ?? void 0 : void 0;
      if (display) nodes.assertIsNode(display, "structFieldDisplayNode");
      return nodes.structFieldTypeNode({ ...node, type, defaultValue, display });
    };
  }
  if (keys.includes("structTypeNode")) {
    visitor.visitStructType = function visitStructType(node) {
      return nodes.structTypeNode(
        (node.fields ?? []).map(visit2(this)).filter(nodes.removeNullAndAssertIsNodeFilter("structFieldTypeNode"))
      );
    };
  }
  if (keys.includes("tupleTypeNode")) {
    visitor.visitTupleType = function visitTupleType(node) {
      return nodes.tupleTypeNode(
        (node.items ?? []).map(visit2(this)).filter(nodes.removeNullAndAssertIsNodeFilter(nodes.TYPE_NODES))
      );
    };
  }
  ...
  if (keys.includes("arrayValueNode")) {
    visitor.visitArrayValue = function visitArrayValue(node) {
      return nodes.arrayValueNode(
        (node.items ?? []).map(visit2(this)).filter(nodes.removeNullAndAssertIsNodeFilter(nodes.VALUE_NODES))
      );
    };
  }
  ...
  if (keys.includes("injectedValueNode")) {
    visitor.visitInjectedValue = function visitInjectedValue(node) {
      ...
      if (fallback) nodes.assertIsNode(fallback, nodes.VALUE_NODES);
      return nodes.injectedValueNode({ ...node, fallback });
    };
  }
  ...
  if (keys.includes("constantPdaSeedNode")) {
    visitor.visitConstantPdaSeed = function visitConstantPdaSeed(node) {
      ...
      const value = visit2(this)(node.value);
      if (value === null) return null;
      nodes.assertIsNode(value, ["programIdValueNode", ...nodes.VALUE_NODES]);
  ...
  if (keys.includes("amountNumberDisplayNode")) {
    visitor.visitAmountNumberDisplay = function visitAmountNumberDisplay(node) {
      ...
      if (decimals) nodes.assertIsNode(decimals, ["numberValueNode", "injectedValueNode"]);
      const unit = node.unit ? visit2(this)(node.unit) ?? void 0 : void 0;
      if (unit) nodes.assertIsNode(unit, ["stringValueNode", "injectedValueNode"]);
      return nodes.amountNumberDisplayNode({ ...node, decimals, unit });
    };
  }
  if (keys.includes("conditionalValueNode")) {
    visitor.visitConditionalValue = function visitConditionalValue(node) {
      ...
      nodes.assertIsNode(condition, ["accountValueNode", "argumentValueNode", "resolverValueNode"]);
      const value = node.value ? visit2(this)(node.value) ?? void 0 : void 0;
      ...
      return nodes.conditionalValueNode({ ...node, condition, value, ifTrue, ifFalse });
    };
  }
  if (keys.includes("pdaSeedValueNode")) {
    visitor.visitPdaSeedValue = function visitPdaSeedValue(node) {
      const value = visit2(this)(node.value);
      if (value === null) return null;
      nodes.assertIsNode(value, ["accountValueNode", "argumentValueNode", ...nodes.VALUE_NODES]);
  ...
  if (keys.includes("pdaValueNode")) {
    visitor.visitPdaValue = function visitPdaValue(node) {
      ...
      if (programId) nodes.assertIsNode(programId, ["accountValueNode", "argumentValueNode"]);
      return nodes.pdaValueNode(
        pda,
        (node.seeds ?? []).map(visit2(this)).filter(nodes.removeNullAndAssertIsNodeFilter("pdaSeedValueNode")),
        programId
      );
    };
  }
  if (keys.includes("resolverValueNode")) {
    visitor.visitResolverValue = function visitResolverValue(node) {
      return nodes.resolverValueNode(node.name, {
        ...node,
        dependsOn: node.dependsOn ? node.dependsOn.map(visit2(this)).filter(nodes.removeNullAndAssertIsNodeFilter(["accountValueNode", "argumentValueNode"])) : void 0
      });
    };
  }
  if (keys.includes("accountNode")) {
    visitor.visitAccount = function visitAccount(node) {
      ...
      nodes.assertIsNestedTypeNode(data, "structTypeNode");
  ...
  if (keys.includes("constantNode")) {
    visitor.visitConstant = function visitConstant(node) {
      ...
      if (value === null) return null;
      nodes.assertIsNode(value, nodes.VALUE_NODES);
      return nodes.constantNode(node.name, type, value, node.docs);
    };
  }
  ...
  if (keys.includes("eventNode")) {
    visitor.visitEvent = function visitEvent(node) {
      const data = visit2(this)(node.data);
      if (data === null) return null;
      nodes.assertIsNode(data, nodes.TYPE_NODES);
  ...
  if (keys.includes("instructionAccountNode")) {
    visitor.visitInstructionAccount = function visitInstructionAccount(node) {
      ...
      if (accountLink) nodes.assertIsNode(accountLink, "accountLinkNode");
      const display = node.display ? visit2(this)(node.display) ?? void 0 : void 0;
      if (display) nodes.assertIsNode(display, "instructionAccountDisplayNode");
      return nodes.instructionAccountNode({ ...node, defaultValue, accountLink, display });
    };
  }
  if (keys.includes("instructionArgumentNode")) {
    visitor.visitInstructionArgument = function visitInstructionArgument(node) {
      ...
      const display = node.display ? visit2(this)(node.display) ?? void 0 : void 0;
      if (display) nodes.assertIsNode(display, "structFieldDisplayNode");
      return nodes.instructionArgumentNode({ ...node, type, defaultValue, display });
    };
  }
  if (keys.includes("instructionByteDeltaNode")) {
    visitor.visitInstructionByteDelta = function visitInstructionByteDelta(node) {
      const value = visit2(this)(node.value);
      if (value === null) return null;
      nodes.assertIsNode(value, ["accountLinkNode", "argumentValueNode", "numberValueNode", "resolverValueNode"]);
  ...
  if (keys.includes("instructionNode")) {
    visitor.visitInstruction = function visitInstruction(node) {
      ...
      const display = node.display ? visit2(this)(node.display) ?? void 0 : void 0;
      if (display) nodes.assertIsNode(display, "instructionDisplayNode");
      return nodes.instructionNode({
        ...
        provides: node.provides ? node.provides.map(visit2(this)).filter(nodes.removeNullAndAssertIsNodeFilter("providedNode")) : void 0,
        display,
        plugins: node.plugins ? node.plugins.map(visit2(this)).filter(nodes.removeNullAndAssertIsNodeFilter("pluginNode")) : void 0
      });
    };
  }
  if (keys.includes("instructionRemainingAccountsNode")) {
    visitor.visitInstructionRemainingAccounts = function visitInstructionRemainingAccounts(node) {
      ...
      const display = node.display ? visit2(this)(node.display) ?? void 0 : void 0;
      if (display) nodes.assertIsNode(display, "instructionAccountDisplayNode");
      return nodes.instructionRemainingAccountsNode(value, { ...node, display });
    };
  }
  ...
  if (keys.includes("programNode")) {
    visitor.visitProgram = function visitProgram(node) {
      return nodes.programNode({
        ...
        constants: (node.constants ?? []).map(visit2(this)).filter(nodes.removeNullAndAssertIsNodeFilter("constantNode")),
        ...
        events: (node.events ?? []).map(visit2(this)).filter(nodes.removeNullAndAssertIsNodeFilter("eventNode")),
  ...
  if (keys.includes("providedNode")) {
    visitor.visitProvided = function visitProvided(node) {
      ...
      if (value === null) return null;
      nodes.assertIsNode(value, nodes.REGISTERED_NODE_KINDS);
      return nodes.providedNode(node.name, value);
    };
  }
...
function identityVisitor2(options = {}) {
  ...
  if (keys.includes("enumStructVariantTypeNode")) {
    overrides.visitEnumStructVariantType = function visitEnumStructVariantType(node, { self }) {
      ...
      return nodes.enumStructVariantTypeNode(node.name, newStruct, node.discriminator);
    };
  }
  if (keys.includes("enumTupleVariantTypeNode")) {
    overrides.visitEnumTupleVariantType = function visitEnumTupleVariantType(node, { self }) {
      ...
      return nodes.enumTupleVariantTypeNode(node.name, newTuple, node.discriminator);
    };
  }
  if (keys.includes("hiddenPrefixTypeNode")) {
    overrides.visitHiddenPrefixType = function visitHiddenPrefixType(node, { self }) {
      ...
      if (type === null) return null;
      nodes.assertIsNode(type, nodes.TYPE_NODES);
      const prefix = (node.prefix ?? []).map(visit2(self)).filter(nodes.removeNullAndAssertIsNodeFilter("constantValueNode"));
  ...
  if (keys.includes("hiddenSuffixTypeNode")) {
    overrides.visitHiddenSuffixType = function visitHiddenSuffixType(node, { self }) {
      ...
      if (type === null) return null;
      nodes.assertIsNode(type, nodes.TYPE_NODES);
      const suffix = (node.suffix ?? []).map(visit2(self)).filter(nodes.removeNullAndAssertIsNodeFilter("constantValueNode"));
  ...
  if (keys.includes("resolverValueNode")) {
    overrides.visitResolverValue = function visitResolverValue(node, { self }) {
      const dependsOn = (node.dependsOn ?? []).map(visit2(self)).filter(nodes.removeNullAndAssertIsNodeFilter(["accountValueNode", "argumentValueNode"]));
      return nodes.resolverValueNode(node.name, {
        ...node,
...
exports.NODE_TEST_PATHS = NODE_TEST_PATHS;
...
exports.identityVisitor = identityVisitor2;
