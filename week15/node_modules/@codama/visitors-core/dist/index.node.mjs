import { REGISTERED_NODE_KINDS, pascalCase, isNode, assertIsNode, REGISTERED_TYPE_NODE_KINDS, isScalarEnum, VALUE_NODES, accountValueNode, argumentValueNode, getAllInstructionArguments, enumEmptyVariantTypeNode, enumStructVariantTypeNode, enumTupleVariantTypeNode, TYPE_NODES, removeNullAndAssertIsNodeFilter, hiddenPrefixTypeNode, hiddenSuffixTypeNode, resolverValueNode, assertIsNestedTypeNode, amountTypeNode, COUNT_NODES, arrayTypeNode, booleanTypeNode, dateTimeTypeNode, enumTypeNode, ENUM_VARIANT_TYPE_NODES, fixedSizeTypeNode, mapTypeNode, numberTypeNode, optionTypeNode, postOffsetTypeNode, preOffsetTypeNode, remainderOptionTypeNode, sentinelTypeNode, setTypeNode, sizePrefixTypeNode, solAmountTypeNode, stringTypeNode, structFieldTypeNode, structTypeNode, tupleTypeNode, zeroableOptionTypeNode, arrayValueNode, constantValueNode, enumValueNode, injectedValueNode, mapEntryValueNode, mapValueNode, setValueNode, someValueNode, structFieldValueNode, structValueNode, tupleValueNode, accountLinkNode, definedTypeLinkNode, instructionAccountLinkNode, instructionArgumentLinkNode, instructionLinkNode, pdaLinkNode, constantPdaSeedNode, variablePdaSeedNode, prefixedCountNode, constantDiscriminatorNode, amountNumberDisplayNode, INSTRUCTION_INPUT_VALUE_NODES, conditionalValueNode, pdaSeedValueNode, pdaValueNode, accountNode, DISCRIMINATOR_NODES, constantNode, definedTypeNode, eventNode, instructionAccountNode, instructionArgumentNode, instructionByteDeltaNode, instructionNode, instructionRemainingAccountsNode, pdaNode, PDA_SEED_NODES, programNode, providedNode, rootNode, camelCase } from '@codama/nodes';
import { CodamaError, CODAMA_ERROR__UNRECOGNIZED_NODE_KIND, CODAMA_ERROR__VISITORS__CANNOT_EXTEND_MISSING_VISIT_FUNCTION, CODAMA_ERROR__VISITORS__CANNOT_REMOVE_LAST_PATH_IN_NODE_STACK, CODAMA_ERROR__LINKED_NODE_NOT_FOUND, CODAMA_ERROR__VISITORS__CYCLIC_DEPENDENCY_DETECTED_WHEN_RESOLVING_INSTRUCTION_DEFAULT_VALUES, CODAMA_ERROR__VISITORS__CANNOT_USE_OPTIONAL_ACCOUNT_AS_PDA_SEED_VALUE, CODAMA_ERROR__VISITORS__INVALID_INSTRUCTION_DEFAULT_VALUE_DEPENDENCY } from '@codama/errors';
...
// src/generated/identityVisitor.ts
function identityVisitor(options = {}) {
  ...
  if (keys.includes("arrayTypeNode")) {
    visitor.visitArrayType = function visitArrayType(node) {
      const count = visit2(this)(node.count);
  ...
  if (keys.includes("enumEmptyVariantTypeNode")) {
    visitor.visitEnumEmptyVariantType = function visitEnumEmptyVariantType(node) {
      const display = node.display ? visit2(this)(node.display) ?? void 0 : void 0;
  ...
  if (keys.includes("enumStructVariantTypeNode")) {
    visitor.visitEnumStructVariantType = function visitEnumStructVariantType(node) {
      const struct = visit2(this)(node.struct);
  ...
  if (keys.includes("enumTupleVariantTypeNode")) {
    visitor.visitEnumTupleVariantType = function visitEnumTupleVariantType(node) {
      const tuple = visit2(this)(node.tuple);
  ...
  if (keys.includes("hiddenPrefixTypeNode")) {
    visitor.visitHiddenPrefixType = function visitHiddenPrefixType(node) {
      ...
      return hiddenPrefixTypeNode(
        type,
        (node.prefix ?? []).map(visit2(this)).filter(removeNullAndAssertIsNodeFilter("constantValueNode"))
      );
    };
  }
  if (keys.includes("hiddenSuffixTypeNode")) {
    visitor.visitHiddenSuffixType = function visitHiddenSuffixType(node) {
      ...
      return hiddenSuffixTypeNode(
        type,
        (node.suffix ?? []).map(visit2(this)).filter(removeNullAndAssertIsNodeFilter("constantValueNode"))
      );
    };
  }
  ...
  if (keys.includes("structTypeNode")) {
    visitor.visitStructType = function visitStructType(node) {
      return structTypeNode(
        (node.fields ?? []).map(visit2(this)).filter(removeNullAndAssertIsNodeFilter("structFieldTypeNode"))
      );
    };
  }
  if (keys.includes("tupleTypeNode")) {
    visitor.visitTupleType = function visitTupleType(node) {
      return tupleTypeNode(
        (node.items ?? []).map(visit2(this)).filter(removeNullAndAssertIsNodeFilter(TYPE_NODES))
      );
    };
  }
  ...
  if (keys.includes("arrayValueNode")) {
    visitor.visitArrayValue = function visitArrayValue(node) {
      return arrayValueNode(
        (node.items ?? []).map(visit2(this)).filter(removeNullAndAssertIsNodeFilter(VALUE_NODES))
      );
    };
  }
  ...
  if (keys.includes("injectedValueNode")) {
    visitor.visitInjectedValue = function visitInjectedValue(node) {
      const fallback = node.fallback ? visit2(this)(node.fallback) ?? void 0 : void 0;
  ...
  if (keys.includes("amountNumberDisplayNode")) {
    visitor.visitAmountNumberDisplay = function visitAmountNumberDisplay(node) {
      const decimals = node.decimals ? visit2(this)(node.decimals) ?? void 0 : void 0;
      if (decimals) assertIsNode(decimals, ["numberValueNode", "injectedValueNode"]);
      const unit = node.unit ? visit2(this)(node.unit) ?? void 0 : void 0;
  ...
  if (keys.includes("pdaValueNode")) {
    visitor.visitPdaValue = function visitPdaValue(node) {
      ...
      const programId = node.programId ? visit2(this)(node.programId) ?? void 0 : void 0;
      if (programId) assertIsNode(programId, ["accountValueNode", "argumentValueNode"]);
      return pdaValueNode(
        pda,
        (node.seeds ?? []).map(visit2(this)).filter(removeNullAndAssertIsNodeFilter("pdaSeedValueNode")),
        programId
      );
    };
  }
  if (keys.includes("resolverValueNode")) {
    visitor.visitResolverValue = function visitResolverValue(node) {
      return resolverValueNode(node.name, {
        ...node,
        dependsOn: node.dependsOn ? node.dependsOn.map(visit2(this)).filter(removeNullAndAssertIsNodeFilter(["accountValueNode", "argumentValueNode"])) : void 0
      });
    };
  }
  ...
  if (keys.includes("instructionAccountNode")) {
    visitor.visitInstructionAccount = function visitInstructionAccount(node) {
      ...
      const accountLink = node.accountLink ? visit2(this)(node.accountLink) ?? void 0 : void 0;
  ...
  if (keys.includes("instructionNode")) {
    visitor.visitInstruction = function visitInstruction(node) {
      ...
      return instructionNode({
        ...
        provides: node.provides ? node.provides.map(visit2(this)).filter(removeNullAndAssertIsNodeFilter("providedNode")) : void 0,
        display,
        plugins: node.plugins ? node.plugins.map(visit2(this)).filter(removeNullAndAssertIsNodeFilter("pluginNode")) : void 0
      });
    };
  }
  ...
  if (keys.includes("programNode")) {
    visitor.visitProgram = function visitProgram(node) {
      return programNode({
        ...
        constants: (node.constants ?? []).map(visit2(this)).filter(removeNullAndAssertIsNodeFilter("constantNode")),
        ...
        events: (node.events ?? []).map(visit2(this)).filter(removeNullAndAssertIsNodeFilter("eventNode")),
  ...
  if (keys.includes("providedNode")) {
    visitor.visitProvided = function visitProvided(node) {
      const value = visit2(this)(node.node);
      if (value === null) return null;
...
function identityVisitor2(options = {}) {
  const keys = options.keys ?? REGISTERED_NODE_KINDS;
  const base = identityVisitor(options);
  const overrides = {};
  const visit2 = (v) => (node) => keys.includes(node.kind) ? visit(node, v) : Object.freeze({ ...node });
  if (keys.includes("enumStructVariantTypeNode")) {
    overrides.visitEnumStructVariantType = function visitEnumStructVariantType(node, { self }) {
      const newStruct = visit2(self)(node.struct);
      if (!newStruct) {
  ...
  if (keys.includes("enumTupleVariantTypeNode")) {
    overrides.visitEnumTupleVariantType = function visitEnumTupleVariantType(node, { self }) {
      const newTuple = visit2(self)(node.tuple);
      if (!newTuple) {
        return enumEmptyVariantTypeNode(node.name);
      }
  ...
  if (keys.includes("hiddenPrefixTypeNode")) {
    overrides.visitHiddenPrefixType = function visitHiddenPrefixType(node, { self }) {
      const type = visit2(self)(node.type);
      if (type === null) return null;
      assertIsNode(type, TYPE_NODES);
      const prefix = (node.prefix ?? []).map(visit2(self)).filter(removeNullAndAssertIsNodeFilter("constantValueNode"));
  ...
  if (keys.includes("hiddenSuffixTypeNode")) {
    overrides.visitHiddenSuffixType = function visitHiddenSuffixType(node, { self }) {
      ...
      if (type === null) return null;
      assertIsNode(type, TYPE_NODES);
      const suffix = (node.suffix ?? []).map(visit2(self)).filter(removeNullAndAssertIsNodeFilter("constantValueNode"));
  ...
  if (keys.includes("conditionalValueNode")) {
    overrides.visitConditionalValue = function visitConditionalValue(node, { next }) {
      ...
      if (visited.ifTrue === void 0 && visited.ifFalse === void 0) return null;
      return visited;
    };
  }
  if (keys.includes("resolverValueNode")) {
    overrides.visitResolverValue = function visitResolverValue(node, { self }) {
      const dependsOn = (node.dependsOn ?? []).map(visit2(self)).filter(removeNullAndAssertIsNodeFilter(["accountValueNode", "argumentValueNode"]));
      return resolverValueNode(node.name, {
        ...node,
  ...
  return extendVisitor(base, overrides);
}
...
function mergeVisitor(leafValue, merge, options = {}) {
  ...
  if (keys.includes("enumEmptyVariantTypeNode")) {
    visitor.visitEnumEmptyVariantType = function visitEnumEmptyVariantType(node) {
      return merge(node, node.display ? visit2(this)(node.display) : []);
    };
  }
  if (keys.includes("enumStructVariantTypeNode")) {
    visitor.visitEnumStructVariantType = function visitEnumStructVariantType(node) {
      return merge(node, [...visit2(this)(node.struct), ...node.display ? visit2(this)(node.display) : []]);
    };
  }
  if (keys.includes("enumTupleVariantTypeNode")) {
    visitor.visitEnumTupleVariantType = function visitEnumTupleVariantType(node) {
      return merge(node, [...visit2(this)(node.tuple), ...node.display ? visit2(this)(node.display) : []]);
    };
  }
  if (keys.includes("enumTypeNode")) {
    visitor.visitEnumType = function visitEnumType(node) {
  ...
  if (keys.includes("structFieldTypeNode")) {
    visitor.visitStructFieldType = function visitStructFieldType(node) {
      return merge(node, [
        ...
        ...node.display ? visit2(this)(node.display) : []
      ]);
    };
  }
  if (keys.includes("structTypeNode")) {
  ...
  if (keys.includes("injectedValueNode")) {
    visitor.visitInjectedValue = function visitInjectedValue(node) {
      return merge(node, node.fallback ? visit2(this)(node.fallback) : []);
    };
  }
  if (keys.includes("mapEntryValueNode")) {
    visitor.visitMapEntryValue = function visitMapEntryValue(node) {
  ...
  if (keys.includes("amountNumberDisplayNode")) {
    visitor.visitAmountNumberDisplay = function visitAmountNumberDisplay(node) {
      return merge(node, [
        ...node.decimals ? visit2(this)(node.decimals) : [],
        ...node.unit ? visit2(this)(node.unit) : []
      ]);
    };
  }
  if (keys.includes("conditionalValueNode")) {
  ...
  if (keys.includes("pdaValueNode")) {
    visitor.visitPdaValue = function visitPdaValue(node) {
      return merge(node, [
        ...visit2(this)(node.pda),
        ...(node.seeds ?? []).flatMap(visit2(this)),
        ...node.programId ? visit2(this)(node.programId) : []
      ]);
    };
  }
  if (keys.includes("resolverValueNode")) {
  ...
  if (keys.includes("eventNode")) {
    visitor.visitEvent = function visitEvent(node) {
      return merge(node, [...visit2(this)(node.data), ...(node.discriminators ?? []).flatMap(visit2(this))]);
    };
  }
  if (keys.includes("instructionAccountNode")) {
    visitor.visitInstructionAccount = function visitInstructionAccount(node) {
      return merge(node, [
        ...node.defaultValue ? visit2(this)(node.defaultValue) : [],
        ...node.accountLink ? visit2(this)(node.accountLink) : [],
        ...node.display ? visit2(this)(node.display) : []
      ]);
    };
  }
  ...
  if (keys.includes("instructionNode")) {
    visitor.visitInstruction = function visitInstruction(node) {
      return merge(node, [
        ...
        ...(node.provides ?? []).flatMap(visit2(this)),
        ...node.display ? visit2(this)(node.display) : [],
        ...(node.plugins ?? []).flatMap(visit2(this))
      ]);
    };
  }
  if (keys.includes("instructionRemainingAccountsNode")) {
    visitor.visitInstructionRemainingAccounts = function visitInstructionRemainingAccounts(node) {
      return merge(node, [...visit2(this)(node.value), ...node.display ? visit2(this)(node.display) : []]);
    };
  }
  if (keys.includes("pdaNode")) {
    visitor.visitPda = function visitPda(node) {
  ...
  if (keys.includes("programNode")) {
    visitor.visitProgram = function visitProgram(node) {
      return merge(node, [
        ...
        ...(node.accounts ?? []).flatMap(visit2(this)),
        ...(node.events ?? []).flatMap(visit2(this)),
        ...(node.instructions ?? []).flatMap(visit2(this)),
        ...(node.definedTypes ?? []).flatMap(visit2(this)),
        ...(node.errors ?? []).flatMap(visit2(this)),
        ...(node.constants ?? []).flatMap(visit2(this))
      ]);
    };
  }
  if (keys.includes("providedNode")) {
    visitor.visitProvided = function visitProvided(node) {
      return merge(node, visit2(this)(node.node));
    };
  }
  if (keys.includes("rootNode")) {
    visitor.visitRoot = function visitRoot(node) {
      return merge(node, [...visit2(this)(node.program), ...(node.additionalPrograms ?? []).flatMap(visit2(this))]);
    };
  }
  return visitor;
}

// src/generated/nodeTestPaths.ts
var NODE_TEST_PATHS = {
...
function getResolvedInstructionInputsVisitor(options = {}) {
  ...
  function resolveInstructionInput(instruction, input) {
    if (isNode(input, "instructionAccountNode") && visitedAccounts.has(input.name) || isNode(input, "instructionArgumentNode") && visitedArgs.has(input.name)) {
      return;
    }
  ...
  function resolveInstructionAccount(instruction, account) {
    ...
    const localResolved = {
      ...
      isPda: getAllInstructionArguments(instruction).some(
        (argument) => isNode(argument.defaultValue, "accountBumpValueNode") && argument.defaultValue.name === account.name
      ),
...
function nonNullableIdentityVisitor(options = {}) {
  return identityVisitor2(options);
}
...
var LinkableDictionary = class {
  ...
  getOrCreateProgramDictionary(linkablePath) {
    const linkableNode = getLastNodeFromPath(linkablePath);
    const programNode2 = isNode(linkableNode, "programNode") ? linkableNode : findProgramNodeFromPath(linkablePath);
  ...
  getOrCreateInstructionDictionary(programDictionary, linkablePath) {
    const linkableNode = getLastNodeFromPath(linkablePath);
    const instructionNode2 = isNode(linkableNode, "instructionNode") ? linkableNode : findInstructionNodeFromPath(linkablePath);
...
export { LINKABLE_NODES, LinkableDictionary, NODE_TEST_PATHS, NodeStack, assertIsNodePath, bottomUpTransformerVisitor, consoleLogVisitor, deduplicateInstructionDependencies, deleteNodesVisitor, extendVisitor, findFirstNodeFromPath, findInstructionNodeFromPath, findLastNodeFromPath, findProgramNodeFromPath, getByteSizeVisitor, getConjunctiveNodeSelectorFunction, getDebugStringVisitor, getInstructionDependencies, getLastNodeFromPath, getMaxByteSizeVisitor, getNodePathUntilLastNode, getNodeSelectorFunction, getRecordLinkablesVisitor, getResolvedInstructionInputsVisitor, getUniqueHashStringVisitor, getVisitFunctionName, identityVisitor2 as identityVisitor, interceptFirstVisitVisitor, interceptVisitor, isFilledNodePath, isNodePath, mapVisitor, mergeVisitor, nodePathToString, nodePathToStringArray, nonNullableIdentityVisitor, pipe, recordLinkablesOnFirstVisitVisitor, recordNodeStackVisitor, removeDocsVisitor, rootNodeVisitor, singleNodeVisitor, staticVisitor, tapVisitor, topDownTransformerVisitor, visit, visitOrElse, voidVisitor };
