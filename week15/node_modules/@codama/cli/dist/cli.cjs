...
var packageJson;
var packageJsonChecked = false;
async function tryGetPackageJson() {
...
async function getPackageJsonDependencies(options = {}) {
  const packageJson2 = await tryGetPackageJson();
...
// src/utils/nodes.ts
var NODES_FROM_ANCHOR = { from: "@codama/nodes-from-anchor", item: "rootNodeFromAnchor" };
async function getRootNodeFromIdl(idl, options = {}) {
  ...
  const rootNodeFromAnchor = await tryImportRootNodeFromAnchor() ?? await resolveRootNodeFromAnchor(options.npxCommandArgs);
  return rootNodeFromAnchor(idl);
}
async function resolveRootNodeFromAnchor(npxCommandArgs = ["[...]"]) {
  if (!await tryGetPackageJson()) {
    throw new CliError("Anchor IDL support is not available.", [
      `${import_picocolors5.default.bold("Missing dependency")}: ${NODES_FROM_ANCHOR.from}`,
      "No package.json was found, so Codama did not install dependencies in this directory.",
      `${import_picocolors5.default.bold("Re-run with")}: ${import_picocolors5.default.yellow(getNpxCommand(npxCommandArgs))}`
    ]);
  }
  return await installRootNodeFromAnchor();
}
function getNpxCommand(commandArgs) {
  return ["npx", "-p", "codama", "-p", NODES_FROM_ANCHOR.from, "codama", ...commandArgs].join(" ");
}
async function installRootNodeFromAnchor() {
  ...
  return await importModuleItem(NODES_FROM_ANCHOR);
}
async function tryImportRootNodeFromAnchor() {
  try {
    return await importModuleItem(NODES_FROM_ANCHOR);
  } catch (error) {
    if (isMissingModuleError(error)) return void 0;
    throw error;
  }
}
function isMissingModuleError(error) {
  for (let current = error; current != null; current = current.cause) {
    const { code } = current;
...
async function getVisitorFromModuleItem(identifier, moduleItem, visitorConfig) {
  ...
  throw new CliError(`Invalid visitor. Expected a visitor or a function returning a visitor.`, [
    `${import_picocolors6.default.bold("Visitor")}: ${identifier}`,
    `${import_picocolors6.default.bold("Path")}: ${path3}`,
    ...item ? [`${import_picocolors6.default.bold("Item")}: ${item}`] : []
  ]);
}
...
async function doInit(explicitOutput, options) {
  ...
  if (!options.force && await canRead(output)) {
    throw new CliError(`Configuration file already exists.`, [`${import_picocolors7.default.bold("Path")}: ${output}`]);
  }
  logBanner();
  ...
  console.log();
  logSuccess(import_picocolors7.default.bold("Configuration file created."), [`${import_picocolors7.default.bold("Path")}: ${output}`]);
}
      ...
      },
      {
        ...
        onRender() {
          if (configFileType === "gill") {
            ...
            jsChoice.description = import_picocolors7.default.yellow("Required with --gill option.");
...
async function getConfig(options) {
  ...
  if (!configFile || typeof configFile !== "object") {
    throw new CliError(`Invalid configuration file.`, [`${import_picocolors8.default.bold("Path")}: ${configPath}`]);
  }
...
async function parseConfig(config, configPath, options) {
  ...
  const idls = await Promise.all([
    loadIdl(idlPath, "IDL"),
    ...additionalIdlPaths.map((path3) => loadIdl(path3, "additional IDL"))
  ]);
  const [mainIdl] = idls;
...
async function loadIdl(from, identifier) {
  const content = await importModuleItem({ identifier, from });
  return { content, rootNode: await getRootNodeFromIdl(content) };
}
function mergeRootNodes(rootNodes) {
  const [head, ...tail] = rootNodes;
  if (tail.length === 0) {
    return head;
  }
  return { ...head, additionalPrograms: [...head.additionalPrograms ?? [], ...tail.flatMap(import_nodes2.getAllPrograms)] };
}
...
function checkMissingScripts(parsedConfig, scripts) {
  ...
  const items = [
    `${import_picocolors9.default.bold(scriptPluralized)}: ${missingScripts.join(", ")}`,
    ...parsedConfig.configPath ? [`${import_picocolors9.default.bold("Path")}: ${parsedConfig.configPath}`] : []
  ];
...
function setConvertCommand(program2) {
  program2.command("convert").argument("<idlPath>", "Path to the input IDL file (e.g., Anchor or supported standard)").argument("<outPath>", "Path for the output Codama IDL file").description("Convert a supported IDL file to Codama IDL format").action(doConvert);
}
async function doConvert(idlPath, outPath) {
...
async function runProgram(program2, argv, parseOptions) {
  try {
    ...
    logError(import_picocolors10.default.bold(error.message), error.items ?? []);
    process.exitCode = 1;
  }
}
function createProgram(internalOptions) {
  const program2 = (0, import_commander.createCommand)().version("1.6.0").allowExcessArguments(false).configureHelp({ showGlobalOptions: true, sortOptions: true, sortSubcommands: true });
  setProgramOptions(program2);
  setInitCommand(program2);
  setRunCommand(program2);
  setConvertCommand(program2);
