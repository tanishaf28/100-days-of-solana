import { titleCase, snakeCase, pascalCase, kebabCase, capitalize, camelCase, REGISTERED_TYPE_NODE_KINDS, REGISTERED_VALUE_NODE_KINDS, isNodeFilter, resolveNestedTypeNode, structTypeNodeFromInstructionArgumentNodes, isNode, isScalarEnum, structTypeNode, structFieldTypeNode, getAllPrograms, getAllPdas, getAllAccounts, getAllInstructionsWithSubs, getAllDefinedTypes, definedTypeLinkNode, definedTypeNode, isDataEnum, accountValueNode, argumentValueNode, parseOptionalAccountStrategy, VALUE_NODES, getAllInstructionArguments, constantDiscriminatorNode, constantValueNodeFromBytes, constantValueNode, assertIsNode } from '@codama/nodes';
...
function getConstantDiscriminatorConstantFragment(discriminatorNode, scope) {
  ...
  const typeManifest2 = visit(discriminatorNode.constant.type, typeManifestVisitor);
...
function getFieldDiscriminatorConstantFragment(discriminatorNode, scope) {
  ...
  const typeManifest2 = visit(field.type, typeManifestVisitor);
  const encoder = typeManifest2.encoder;
  const { value, valueType } = resolveDiscriminatorValue(
...
function getFieldConditionFragment(discriminator, scope) {
  ...
  if (isNode(field.type, "arrayTypeNode") && isNode(field.type.item, "numberTypeNode") && field.type.item.format === "u8" && isNode(field.type.count, "fixedCountNode") && isNode(field.defaultValue, "arrayValueNode") && defaultValueItems.every(isNodeFilter("numberValueNode"))) {
